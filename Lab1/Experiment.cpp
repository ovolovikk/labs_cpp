...
void Experiment::runSingle(int n, int k, int trials, unsigned int seed) {
  const auto [mean, median] = Statistics::runExperiment(n, k, trials, seed);
...
void Experiment::runSweep(int n, int trials, unsigned int seed, int k_max,
                          int step, const std::string& output_file) {
  if (k_max < 0) {
  ...
  std::cout << "Sweep: n = " << n << ", trials = " << trials
            << ", seed =" << seed << ", k from 0 to " << k_max << " step "
            << step << "\n";

  for (int k = 0; k <= k_max; k += step) {
    const auto [mean, median] =
        Statistics::runExperiment(n, k, trials, seed + k);
...
void Experiment::printUsage(const char* prog) {
  std::cout << "Usage:\n"
            ...
            << "  " << prog
            << " n k trials [seed] --sweep k_max step out.csv\n\n"
            << "Examples:\n"
            << "  " << prog << " 20 10 10000\n"
